...
// Grid object
Grid manager(&xmotor, &ymotor, &ymotorTwo, 4094      , MAGNET    , ACTUATOR    , 12 * 800 + 912 - 2047, 12 * 800 + 125       , -4094, -4094             , -8460 + 100, -8189 , 8360 - 50, -8189 + 50);
...
JoystickController player1;
JoystickController player2;
ComputerController computer1;
DemonstrationController demo;

Game twoPlayerGame(&manager, &player1, &player2);
Game onePlayerGame(&manager, &player1, &computer1);
Game demonstrationGame(&manager, &demo, &demo);
...
void setup(){ /* Setup code here. */
  ...
  player1.assign(&joystick1, &endTurn1);
  player2.assign(&joystick2, &endTurn2);
  demo.assign(allGames, 3);
  manager.setSpeed(800 * 20); // One inch per second is 800, so all of the motors are set to 3x.
  xmotor.setSpeed(800 * 20); // Override the manager; change the X motor to 4x because it can go faster without breaky-breaky-ing.
  manager.zero();
  randomSeed(analogRead(A0));
  twoPlayerGame.assignLights(&lights);
  twoPlayerGame.newGame();
  onePlayerGame.assignLights(&lights);
  onePlayerGame.newGame();
  demonstrationGame.assignLights(&lights);
  demonstrationGame.newGame();
  lights.startup();
  lights.clearBoard();
}
...
void loop(){ /* This is the mainloop. */
  ...
  else if (gameMode.position == 2){
    if (twoPlayerGame.run()){
      playing = false;
    }
  }
  else if (gameMode.position == 1){
    if (onePlayerGame.run()){
      playing = false;
    }
  }
  else if (gameMode.position == 0){
    if (demonstrationGame.run()){
      playing = false;
    }
  }
}
